<?php
	...
	$sql="SELECT content FROM contents WHERE language='" . $lang . "' AND contentname='CopyrightInfo'";
	$result=mysql_query($sql);
	$nrow=mysql_numrows($result);
	
	$smarty->assign('title','Copyright');	
	
	if($nrow>0){
		$smarty->assign('content',mysql_result($result,0,'content'));
		
	}
	
	$smarty->display('legal.html');
