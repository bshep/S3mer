...
// Check post_max_size (http://us3.php.net/manual/en/features.file-upload.php#73762)
	...
	if ((int)$_SERVER['CONTENT_LENGTH'] > $multiplier*(int)$POST_MAX_SIZE && $POST_MAX_SIZE) {
		header("HTTP/1.1 508 Internal Server Error");
...
// Settings
	...
	$extension_whitelist = array("jpg", "gif", "png", "nfo", "img");	// Allowed file extensions
...
// Validate the upload
    ...
    header("HTTP/1.1 507 Internal Server Error");
		//HandleError("No upload found in \$_FILES for " . $upload_name);
		exit(0);
	} else if (isset($_FILES[$upload_name]["error"]) && $_FILES[$upload_name]["error"] != 0) {
    header("HTTP/1.1 506 Internal Server Error");
...
// Process the file
	...
	*/
	...
	$fs = fopen($save_path.'/post.txt','w+');
	fwrite($fs,print_r($_POST,true));
	fclose($fs);
