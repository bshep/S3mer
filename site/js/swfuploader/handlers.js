...
function uploadStart(file){
	try{
		...
		var keyval = "user/" + currentuser.trim() + "/" + file.name;
		swfu.removePostParam('key');
		swfu.addPostParam('key', keyval);
...
function fileDialogComplete(num_files_queued) {
	try {
		if (num_files_queued > 0) {
			this.startUpload();
			modal1.open (); /* Aparece modal window */
		}
	} catch (ex) {
		this.debug(ex);
	}
...
function uploadProgress(fileObj, bytesLoaded) {
	...
	try {
		...
		if (percent === 100) {
			...
			//new Ajax.Request('process_command.php?commandnr=save-media-thumbnail&filename=' + fileObj.name, { asynchronous: false });
...
function uploadComplete(fileObj) {
	try {
		...
		new Ajax.Request('process_command.php?commandnr=save-media-data&filename=' + fileObj.name + '&filesize=' + fileObj.size);
		//progress.SetStatus("Creating thumbnail...");
		//new Ajax.Request('process_command.php?commandnr=save-media-thumbnail&filename=' + fileObj.name, { asynchronous: true });
		...
		if (this.getStats().files_queued > 0) {
			...
			//Recalc el space del usuario
			
			searchImages();
			
			setTimeout('modal1.close ();', 1000); /* Desaparece modal window */
...
function uploadError(fileObj, error_code, message) {
	...
	try {
		switch(error_code) {
			...
			break;
			case -200: /* Skip alert 201 = Successfull Upload */
					break;
			default:	
				alert(message);
