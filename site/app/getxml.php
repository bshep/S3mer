<?php
	...
	$sql = "SELECT player.channel,channel.mediaurl,channel.configurl FROM player INNER JOIN channel on channel.id=player.channel where player.id=" . $playerid . " AND player.owner = '" . $_SESSION['player']['userid'] . "'";
	...
	$sql2 = "select pro from player where id={$playerid}";
	...
	if ($num!=0){	//player found proceding to get xml configuration file for player
		...
		while( $show_row = mysql_fetch_array($show_list) ) {
			...
			$sql2="SELECT showregion.id,showregion.top,showregion.`left`,showregion.width,showregion.height,showregion.fontcolor,showregion.bgcolor1,showregion.rssid,showregion.rssurl,regiontype,url,rsscolor, showregion.mainmedia FROM showregion 
		...
		$sql='SELECT DISTINCT playlist, showregion.mainmedia from regionplaylists INNER JOIN showregion on regionplaylists.regionid=showregion.id INNER JOIN channelschedule ON showregion.showid=channelschedule.showid WHERE channelschedule.channel=' . $channel;
		$playlist_list=mysql_query($sql);
		
		while( $playlist_row = mysql_fetch_array($playlist_list) ) {
			$plid = $playlist_row["playlist"];
			$mainmediaregion = $playlist_row['mainmedia']; 
			...
			if ($playlist_rss[$plid]) {
				...
				$sql2="SELECT media.name,mediatype.mediatype,duration,nomediatype,url FROM playlistitem LEFT OUTER JOIN media ON media.id=playlistitem.mediaid LEFT OUTER JOIN mediatype ON mediatype.id=media.mediatype WHERE playlistitem.playlistid=" . $plid . 
...
*/
			...
			if($mainmediaregion == 1 and $pro != 1){

				$sqlad = "select url from ads order by rand() limit 1";
				$resultad = mysql_query($sqlad);
				
				if($resultad != null){
					
					while($ad_row = mysql_fetch_array($resultad)){
						
						$XML.="\t\t<playlistitem duration=\"" . "0" . 
							"\" type=\"" . "video" . '">' . 
								$ad_row['url'] . 
							"</playlistitem>\n";
