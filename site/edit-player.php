<?php
	...
	$sql="SELECT count(id) as playercount FROM player WHERE id=" . $_GET['playerid'] . " AND owner=" . $loggeduser['id'];
	$result=mysql_query($sql);
	$playercount=0;
	
	if($result!=null){
		$playercount=mysql_result($result,0);
	}
	...
	if($nr>0){
		...
		$sql="UPDATE channelschedule SET temporder=`order` WHERE channel=" . $channelnumber;
		$afr=mysql_query($sql);
	...
	$sql="SELECT *,DATE_FORMAT(lastuptime,'%m/%d/%Y %r') AS lastuptime2 FROM player WHERE id=" . $_GET['playerid'];
	$result=mysql_query($sql);
	$player=array();
	$nr=mysql_numrows($result);
	...
	$smarty->display('edit-player.html');
